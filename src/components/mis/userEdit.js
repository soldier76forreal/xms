...
const UserEdit =(props)=>{
    ...
    return(
        <Fragment>
            {props.editUser.status === true?
                <div className={Style.newInvoice}>
                    <div className={Style.topSection}>
                        <div onClick={()=>{props.setEditUser({status:false , id:''});  history.push('#EditUser');}} className={Style.backBtn}><ArrowBackIosIcon sx={{color:'#000' , fontSize:'30px'}}></ArrowBackIosIcon></div>
                        <div className={Style.topTitle}>ویرایش کاربر</div>
                    </div>
