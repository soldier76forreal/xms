                ...
                const response = await authContext.jwtInst({
                    ...
                    onUploadProgress: data => {                           
                        ...
                        if(Math.round((100 * data.loaded) / data.total) === 100){
                            ...
                            props.setSuccessToast({status:true , msg:'User has been edited'});
                            props.setRefresh(Math.random())
                            const closingSuccessMsgTimeOut = setTimeout(()=>{props.setSuccessToast({status:false , msg:'User has been edited'})}, 3000);
                ...
                for(var j = 0 ; props.targetedEditData.access.length > j ; j++){
                    for(var i = 0 ; webSectionContext.listOfSections.length > i ; i++){
                        if(props.targetedEditData.access[j] === webSectionContext.listOfSections[i].value){
                            tempArr.push({id:webSectionContext.listOfSections[i].value , text:webSectionContext.listOfSections[i].name})
                        }
                    }
                }
            }
        ...
        <Fragment>
            <div style={props.openEditUser === true?{display:'block'}:{display:'none'}}  className={props.openEditUser === true? `${Style.newInvoice} ${Style.fadeIn}` : props.openEditUser === false?`${Style.newInvoice} ${Style.fadeOut}`:null}>
                <div className={Style.topSection}>
                    ...
                    <div className={Style.topTitle}>Edit profile</div>
                </div>
                <div className={Style.ovDiv}  dir='ltr'  style={{maxWidth:'700px' , margin:'0px auto 0px auto'  , overflowY:'scroll' , height:'95vh' , padding:"0px 15px 60px 15px"}}>
                    <div style={{margin:'0px auto 0px auto' }}>        
                        ...
                        <div style={{padding:'60px 10px 0px 10px'}}> 
                            <Row style={{padding:'13px 8px 13px 8px'}}>
                                ...
                                <Col style={{padding:'0px 5px 0px 0px', textAlign:'right'}} xs={6} md={6} lg={6} xl={6} xxl={6}>
                                    {phoneNumber}
                                </Col>
                            </Row>                 
                            <Row>
                                <Col style={{padding:'0px 5px 0px 0px'}} xs={6} md={6} lg={6} xl={6} xxl={6}>
                                    <MuiInput value={name} onChange={(e)=>{setName(e.target.value)}} name='Firs name' type='normal' width='100%'></MuiInput>
                                </Col>
                                <Col style={{padding:'0px 5px 0px 0px'}} xs={6} md={6} lg={6} xl={6} xxl={6}>
                                    <MuiInput value={lastName} onChange={(e)=>{setLastName(e.target.value)}} name='Last name' type='normal' width='100%'></MuiInput>
                                </Col>
                            </Row>
                            <Row>
                                <Col style={{padding:'0px 0px 0px 0px' , marginTop:'15px'}} xs={12} md={12} lg={12} xl={12} xxl={12}>
                                    <MultiSelect value={access} selectType='userAccess' onChange={getAccess} options={webSectionContext.listOfSections} setAccess={setAccess} type='normal' width='100%'></MultiSelect>
                                </Col>
                            </Row>
                            <Row>
