...
const NewUserPortal =(props)=>{
    ...
    const sendData = async() =>{
        ...
        try{
            ...
            history.push('#');
            props.setSuccessToast({status:true , msg:'New user added!'});
            props.setRefresh(Math.random())
            const closingSuccessMsgTimeOut = setTimeout(()=>{props.setSuccessToast({status:false , msg:'New user added'})}, 3000);
            ...
            {props.newUserStatus === true?
                <div className={Style.newInvoice}>
                    <div className={Style.topSection}>
                        ...
                        <div className={Style.topTitle}>New user</div>
                    </div>
                    <div dir='ltr' style={{marginTop:'30px'}} className={Style.formDiv}>
                    {/* <div dir="ltr" className={props.showModal === true ? `${Style.modalDiv} ${Style.fadeIn}` : props.showModal === false ? `${Style.modalDiv} ${Style.fadeOut}` : null}>
                        ...
                        <div  style={{padding:'0px 10px 0px 10px'}}>                  
                            <Row>
                                <Col style={{padding:'0px 5px 0px 0px'}} xs={6} md={6} lg={6} xl={6} xxl={6}>
                                    <MuiInput onChange={(e)=>{setName(e.target.value)}} name='Name' type='normal' width='100%'></MuiInput>
                                </Col>
                                <Col style={{padding:'0px 0px 0px 5px'}} xs={6} md={6} lg={6} xl={6} xxl={6}>
                                    <MuiInput onChange={(e)=>{setLastName(e.target.value)}} name='Last name' type='normal' width='100%'></MuiInput>
                                </Col>
                            </Row>
                            <Row>
                                <Col  style={{padding:'10px 5px 0px 0px'}} xs={4} md={4} lg={4} xl={4} xxl={4}>
                                    <MuiInput name='Confirmation' type='normal' width='100%'></MuiInput>
    
                                </Col>
                                <Col style={{padding:'10px 0px 0px 5px'}} xs={8} md={8} lg={8} xl={8} xxl={8}>
                                    <MuiInput onChange={(e)=>{setPhoneNumber(e.target.value)}} name='Phone number' type='normal' width='100%'></MuiInput>
                                </Col>
                            </Row>
                            <Row>
                                <Col style={{padding:'0px 0px 0px 0px' , marginLeft:'-8px'}} xs={12} md={12} lg={12} xl={12} xxl={12}>
                                    <MuiInput  onChange={(e)=>{setPassword(e.target.value)}} name='Password' type='password' width='100%'></MuiInput>
                                </Col>
                            </Row>
                            <Row>
                                <Col style={{padding:'0px 0px 0px 0px'}} xs={12} md={12} lg={12} xl={12} xxl={12}>
                                    
                                    <MultiSelect selectType='userAccess' onChange={getAccess} options={webSectionContext.listOfSections} setAccess={setAccess} type='normal' width='100%'></MultiSelect>
                                </Col>
                            </Row>
                            <Row>
                                <Col xs={12} md={12} lg={12} xl={12} xxl={12}>
                                    <div className={Style.btnDiv}>
                                        <Button onClick={sendData} sx={{backgroundColor:'rgb(0, 0, 0)' , fontSize:'15px' , color:'#fff'}} variant="contained">Save</Button>
                                    </div>
                                </Col>
                            </Row>
                        </div>
