...
export default function ExpendUserAc(props) {
  ...
  const [validation , setValidation] = useState(false);
    ...
    const deleteModal = () =>{
        Swal.fire({
            title: 'Are you sure?',
            text: "This user will be removed from your user list",
            icon: 'warning',
    ...
    useEffect(() => {
      if(props.data !== undefined){
        ...
        const thirdArray = webSections.listOfSections.filter((elem) => {
          ...
          const remap = thirdArray.map(e=>{
              return {id:e.value , text:e.name}
          })
          setAccess([...remap])
      }
    }, []);
            ...
            >
              <Typography>
                <div style={{width:'100%' , display:'flex' , alignItems:'center'}}>
                    ...
                    />
                    <div>
                        <div style={{fontSize:'18px' , marginLeft:'8px' , padding:'0px'}}>{`${props.data.firstName} ${props.data.lastName}`}</div>
                        <div style={{fontSize:'13px' , color:'rgb(69, 69, 69)' , marginLeft:'8px' , padding:'0px'}}>{`Roles:${props.data.access.map(data=>{
            ...
            <Row>
                      ...
                      <Row style={{width:'100%'}}>
                        <Col style={{padding:'0px'}} xs={12} md={6} lg={6} xl={6} xxl={6}>
                          <div className={Style.validationDiv} >
                            {validation === true ?
                              <div className={Style.validation}>User is <sapn style={{backgroundColor:'#000' , margin:'0px 1px 0px 1px' , color:'#22e151' ,padding:'3px 8px 3px 8px' , borderRadius:'50px' ,fontFamily:'YekanBold'}}>active</sapn> </div>
                              :validation === false ?
                              <div className={Style.validation}>User is <sapn style={{backgroundColor:'#000' ,margin:'0px 1px 0px 1px', color:'#ff1f1f' , padding:'3px 8px 3px 8px' , borderRadius:'50px' ,fontFamily:'YekanBold'}}>not activated</sapn></div>
                            :null}
                            <FormControlLabel
                            sx={{marginLeft:'2px'}}
                              control={
              ...
              <Row>
                  <Col style={{padding:'5px 10px 0px 10px'}} xs={12} md={12} lg={12} xl={12} xxl={12}>
                      <div style={{margin:'0px 7px 2px 0px',  fontSize:'16px'}}>Roles</div>

                      <MultiSelect onChange={getAccess}  value={access} selectType='userAccess'  options={webSections.listOfSections} setAccess={setAccess}  type='normal' width='100%'></MultiSelect>
                  </Col>
              </Row>
