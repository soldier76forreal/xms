...
const NewPreInvoicePortal = (props) =>{
    ...
    return(
        <Fragment>
                ...
                <div style={newInvoice === true?{display:'block'}:{display:'none'}}  className={newInvoice === true? `${Style.newInvoice} ${Style.fadeIn}` : newInvoice === false?`${Style.newInvoice} ${Style.fadeOut}`:null}>
                    <div className={Style.topSection}>
                        <div onClick={()=>{dispatch(actions.toggleNewInvoice()); history.push('#newPreInvoices')}} className={Style.backBtn}><ArrowBackIosIcon className={Style.arrowIcon} sx={{color:'#000' , fontSize:'30px'}}></ArrowBackIosIcon></div>
                        <div className={Style.topTitle}>New invoice</div>
                    </div>
                    <div style={{overflowY:'scroll' , height:'100vh' , padding:'0px 0px 40px 00px'}}>              
