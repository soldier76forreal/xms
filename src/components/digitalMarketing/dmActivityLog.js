...
// Who viewed / downloaded a raw content or ready-to-upload record (and which
// file, for a download). Self-views by the owner are never logged server-side,
// so this only ever shows OTHER people's activity — the useful signal.
// endpointBase: 'raw-contents' | 'ready-to-upload'.
export default function DmActivityLog({ endpointBase, id, T }) {
  ...
  useEffect(() => {
    ...
    authCtx.jwtInst({
      method: 'get',
      url: `${axiosGlobal.defaultTargetApi}/digitalMarketing/${endpointBase}/${id}/activity`,
          ...
          {rows.map((r) => (
            <Box key={r._id} sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              ...
              <Typography sx={{ fontSize: '0.76rem', color: T.TEXT_SEC, flexGrow: 1, minWidth: 0 }} noWrap>
                ...
                {' '}
                {r.action === 'downloaded'
                  ? (r.fileName ? t('dm.downloadedNamedFile', { fileName: r.fileName }) : t('dm.downloadedAFile'))
                  : t('dm.viewedThis')}
              </Typography>
