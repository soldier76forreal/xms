...
import { ArrowBackIos, Business, Call, Cancel, Done, Edit, Email, Facebook, Instagram, LinkedIn, Person, Web, WebStories } from '@mui/icons-material';
...
import { Avatar, Divider, Grid, Icon } from '@mui/material';
...
import FindCustomerType from '../functions/findCustomerType';
...
const ShowCustomerPortal = (props) =>{
    ...
    const [personOrCompany , setPersonOrCompany] = useState(null);
    ...
    const callType =[ {value:'sales' , pr:'Sales'} ,  {value:'requestFollowUp' , pr:'Operation follow up'} ,  {value:'invoiceFollowUp' , pr:'Invoice follow up'} , {value:'customerSatisfaction' , pr:'Customer satisfaction'}]
   ...
   //facebook information states
   const [facebooks , setFacebooks] = useState([{facebook:''}]);
    
   //botim information states
   const [botims , setBotims] = useState([{botim:''}]);
    ...
    useEffect(() => {
        if(props.showCustomer.status === true){
           ...
           if(result[0].customer.personalInformation.firstName !==null){
            ...
            if(result[0].customer.personalInformation.companyName !==null){
                setCompanyName(result[0].customer.personalInformation.companyName)
            }
            ...
            if(result[0].customer.personalInformation.personOrCompany !==null){
                setPersonOrCompany(result[0].customer.personalInformation.personOrCompany)
            }
            ...
            if(result[0].customer.contactInfo.facebooks !==null){
                setFacebooks([...result[0].customer.contactInfo.facebooks])
            }else if(result[0].customer.contactInfo.facebooks ===null){
                setFacebooks(null)
            }

            if(result[0].customer.contactInfo.botims !==null){
                setBotims([...result[0].customer.contactInfo.botims])
            }else if(result[0].customer.contactInfo.botims ===null){
                setBotims(null)
            }
            if(result[0].customer.communication.length !==0){
                var dayByDayArr = Object.keys(SortDayByDay(result[0].customer.communication)).map((key) => [key, SortDayByDay(result[0].customer.communication)[key]]);
    ...
    //facebook onChange 
    const getFacebook = (e) =>{
        const updatedItems = facebooks.map((item,i) => {
            if (parseInt(e.target.name) === parseInt(i)) {
                return { ...item, facebook: e.target.value };
            }
            return item;
        });
        // Update the state with the new array
        setFacebooks([...updatedItems]);
        
    }

    //botim onChange 
    const getBotim = (e) =>{
        const updatedItems = botims.map((item,i) => {
            if (parseInt(e.target.name) === parseInt(i)) {
                return { ...item, botim: e.target.value };
            }
            return item;
        });
        // Update the state with the new array
        setBotims([...updatedItems]);
    }
    //linkedIns onChange 
    const getLinkeIn = (e) =>{
    ...
    const editContactSection = async() =>{
        ...
        const dataToSend = {
            ...
            facebook:checkArray(facebooks ,'facebook'),
            botim:checkArray(botims ,'botim'),
            generatedBy:decoded.id,
            id:props.showCustomer.id
        }
        if(firstName === '' || lastName === '' || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
 
              dispatch(actions.setShowSnackBar({status:true,msg:'To add new customer, you have to enter first name ,last name and phone number.',type:'error'}))
            ...
            try{
                ...
                setLoading(false)
                dispatch(actions.setShowSnackBar({status:true,msg:'Customer information has been edited',type:'success'}))
                ...
                dispatch(actions.setShowSnackBar({status:true,msg:err.response.data,type:'error'}))
    ...
    const editPersonalSection = async() =>{
        ...
        if(personOrCompany === null){
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Select the company type',
              })
              setLoading(false)
        }else if(personOrCompany === 'pr'){
            dataToSend ={
                personalInformation:{
                    firstName : firstName,
                    lastName : lastName,
        ...
        if(personOrCompany === 'pr'){
            if(customerType !== null){
                if(firstName === '' || lastName === '' || firstName === null || lastName === null || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                ...
                dispatch(actions.setShowSnackBar({status:true,msg:'Select the customer type'.response.data,type:'error'}))
            ...
            if(customerType !== null){
                if(companyName === null || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){

                      dispatch(actions.setShowSnackBar({status:true,msg:'To add new customer, you have to enter the company name and phone number.'.response.data,type:'error'}))
        ...
        }else if(personOrCompany === ''){

            dispatch(actions.setShowSnackBar({status:true,msg:'Select person or company'.response.data,type:'error'}))
    ...
    const editAddressAndExplaintionSection = async() =>{
        ...
        if(firstName === '' || lastName === '' || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
            ...
            dispatch(actions.setShowSnackBar({status:true,msg:'To add new customer, you have to enter first name ,last name and phone number.'.response.data,type:'error'}))
    ...
    const editContactInformationFunc = () =>{
        ...
        if(props.showCustomer.status === true){
            ...
            if(result.contactInfo.facebooks !==null && result.contactInfo.facebooks.length !== 0){
                setFacebooks([...result.contactInfo.facebooks])
            }else if(result.contactInfo.facebooks ===null || result.contactInfo.facebooks.length === 0){
                setFacebooks([{facebook:''}])
            }
            if(result.contactInfo.botims !==null && result.contactInfo.botims.length !== 0){
                setBotims([...result.contactInfo.botims])
            }else if(result.contactInfo.botims ===null ||  result.contactInfo.botims.length === 0){
                setBotims([{botim:''}])
            }
        ...
        <Fragment>
            ...
            <div style={props.showCustomer.status === true?{display:'block'}:{display:'none'}}  className={props.showCustomer.status === true? `${Style.newInvoice} ${Style.fadeIn}` : props.showCustomer.status === false?`${Style.newInvoice} ${Style.fadeOut}`:null}>
                ...
                <div className={Style.ovDiv}    style={{maxWidth:'700px' , overflowx:'none' , overflowY:'scroll' , height:'95vh' , position:'relative' , padding:"0px 0px 100px 0px"}}>
                    <div style={{padding:'0px 15px 60px 15px'}}>  
                        ...
                        {listType === 'prInformation' ?
                            <div>
                                <div style={{height:'250px',display:'flex' , justifyContent:'center',alignItems:'center'}}>
                                    {personOrCompany === 'com'?
                                        <div>
                                            <Business sx={{fontSize:'120px'}}></Business>
                                            <div style={{width:'100%', textAlign:'center',fontSize:'30px' , fontFamily:'YekanBold'}}>Company</div>
                                        </div>
                                    :personOrCompany === 'pr'?
                                        <div>
                                            <Person sx={{fontSize:'120px'}}></Person>
                                            <div style={{width:'100%', textAlign:'center',fontSize:'30px' , fontFamily:'YekanBold'}}>Person</div>
                                        </div>
                                    :
                                        <div>
                                            <Person sx={{fontSize:'120px'}}></Person>
                                    ...
                                    <div>
                                        <div className={Style.customerTypes}>
                                            <div className={Style.titlePaddign}>
                                                <BigOneWithDot text="Customer type"></BigOneWithDot>
                                            ...
                                            <Row className="g-0">
                                                <Col style={{padding:'10px 5px 0px 5px' , zIndex:'1001'} } sm={6} md={6} lg={6} xl={6} xxl={6} xs={6}>
                                                    <CustomSelect value={customerType} onChange={(e)=>{setCustomerType(e.id);}} selectType='customerType' placeholder="Customer type"></CustomSelect>
                                                </Col>
                                                <Col style={{padding:'10px 5px 0px 5px' , zIndex:'2000'} } sm={6} md={6} lg={6} xl={6} xxl={6} xs={6}>
                                                    <CustomSelect value={personOrCompany} onChange={(e)=>{setPersonOrCompany(e.id);}} selectType='personOrCompany' placeholder="Person or company"></CustomSelect>
                                                </Col>
                                            </Row>
                                        </div>
                                                ...
                                                :personOrCompany === 'com'?
                                                    <Fragment>
                                                        <Col style={{padding:'10px 5px 0px 5px'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <TextInputNormal value={companyName} onChange={(e)=>{setCompanyName(e.target.value)}}  placeholder="Company name"></TextInputNormal>
                                                        </Col>
                                                    </Fragment>
                                                :
                                                    <Fragment>
                                ...
                                :editPersonalInformation === false?
                                    ...
                                    <div className={Style.personalInformationFirstOne}>
                                        <Row className="g-0">
                                            <Col style={{padding:'10px 5px 0px 5px' , zIndex:'1000'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                <div style={{padding:'10px 0px 10px 0px',borderRadius:'5px', textAlign:'center',backgroundColor:customerType==='cc'?'rgb(112, 236, 139)':customerType==='le'?'rgb(239, 156, 78)':customerType==='pc'?'rgb(219, 219, 219)':'rgb(219, 219, 219)'}}>
                                                    {customerType !== null ?FindCustomerType(customerType):'Lead'}
                                                </div>
                                            </Col>
                                        </Row>
                                        <Row className="g-0">
                                                    ...
                                                    <div style={{display:'flex', alignItems:'center'}}>
                                                        <Grid style={{display:'flex'}} container spacing={0}>
                                                            <Grid style={{ display:'flex', alignItems:'center', justifyContent:'center'}} item xs="auto" >
                                                                {personCountry !== null?
                                                                    <Fragment>
                                                                        <div style={{width:'40px' , borderRadius:'8px' , height:'40px'}}>
                                                            ...
                                                            <Grid style={{ display:'flex', alignItems:'center', justifyContent:'center'}} item xs="auto" >
                                                                ...
                                                                :personOrCompany === 'com'?
                                                                    <span style={{padding:'8px 10px 8px 10px' , borderRadius:'8px' , marginLeft:'10px' , backgroundColor:'white'}}>{companyName}</span>
                                                                :
                                                                    <span style={{padding:'8px 10px 8px 10px' , borderRadius:'8px' , marginLeft:'10px' , backgroundColor:'white'}}>{firstName} {lastName}</span>
                                                                }
                                                            </Grid>
                                                            <Grid style={{display:'flex', backgroundColor:'#EFEFEF', borderRadius:'100px' , alignItems:'center'}} item xs="auto" >
                                                                {personCountry !== null?
                                                                    <Fragment>
                                                                        <span style={{padding:'8px 10px 8px 10px' , borderRadius:'8px' }}>
                                                                            <span style={{color:'rgb(109, 109, 109)' , fontSize:'13px'}}>from </span>
                                                                            <span style={{ fontSize:'13px'}}>{CountriesInFarsi(personCountry)}</span>
                                                                        </span>
                                                                    </Fragment>
                                                                :null}
                                                            </Grid>
                                            ...
                                            <Col style={{padding:'10px 5px 0px 5px'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                <div style={{display:'flex', backgroundColor:'#EFEFEF', borderRadius:'100px' , alignItems:'center'}}>
                                                    ...
                                                    {customerOrigin !== null?
                                                        <Fragment>
                                                            <span style={{padding:'8px 10px 8px 10px' , borderRadius:'8px' }}>
                                                                <span style={{color:'rgb(109, 109, 109)' , fontSize:'13px', marginRight:'5px'}}>Attracted through</span>
                                                                <span style={{fontSize:'14px'}}>{CustomerOrigin(customerOrigin)}</span>
                                                            </span>
                                                        </Fragment>
                                                    :null}
                                                </div>
                                ...
                                {editContactInformation=== true?
                                    <div>
                                        ...
                                        <div> 
                                            <Row className="g-0"  style={{padding:'30px 0px 0px 0px'}}>
                                                <div className={Style.littleTitleDiv}>
                                                    <LittleOneNormal  text='botim'></LittleOneNormal>
                                                    <hr style={{marginRight:'8px' , maxWidth:'15px' , padding:"3px , 0px , 3px , 0px"}} className={Style.dashLineVer}></hr>
                                                </div>
                                                <Col style={{marginBottom:'5px'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                    <IconBotton onClick={()=>{var a = [...botims]; a.push({botim:''}); setBotims([...a])}} color='black' name='New botim url'  text={true} icon={<AddIcon sx={{color:'rgb(231, 231, 231)'}}/>}></IconBotton>
                                                </Col>
                                            </Row>
                                            {botims.map((data , i)=>{
                                                return(
                                                    <Col style={{padding:'5px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                        <div className={Style.phoneNumberMainDiv}>
                                                            <div className={Style.dot}>{i+1}</div>
                                                                <IconBotton onClick={()=>{var temp = [...botims]; temp.splice(i,1); setBotims([...temp]) }} text={false} icon={<DeleteIcon sx={{color:'#EA005A'}}/>}></IconBotton>
                                                            <div className={Style.phoneNumberCountry}>
                                                                <TextInputNormal value={data.botim} name={i} onChange={getBotim} placeholder='botim url'></TextInputNormal>
                                                            </div>
                                                        </div>
                                                    </Col>
                                                )
                                        ...
                                        <div> 
                                            <Row className="g-0"  style={{padding:'30px 0px 0px 0px'}}>
                                                <div className={Style.littleTitleDiv}>
                                                    <LittleOneNormal  text='facebook'></LittleOneNormal>
                                                    <hr style={{marginRight:'8px' , maxWidth:'15px' , padding:"3px , 0px , 3px , 0px"}} className={Style.dashLineVer}></hr>
                                                </div>
                                                <Col style={{marginBottom:'5px'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                    <IconBotton onClick={()=>{var a = [...facebooks]; a.push({facebook:''}); setFacebooks([...a])}} color='black' name='New facebook url'  text={true} icon={<AddIcon sx={{color:'rgb(231, 231, 231)'}}/>}></IconBotton>
                                                </Col>
                                            </Row>
                                            {facebooks.map((data , i)=>{
                                                return(
                                                    <Col style={{padding:'5px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                        <div className={Style.phoneNumberMainDiv}>
                                                            <div className={Style.dot}>{i+1}</div>
                                                                <IconBotton onClick={()=>{var temp = [...facebooks]; temp.splice(i,1); setFacebooks([...temp]) }} text={false} icon={<DeleteIcon sx={{color:'#EA005A'}}/>}></IconBotton>
                                                            <div className={Style.phoneNumberCountry}>
                                                                <TextInputNormal value={data.facebook} name={i} onChange={getFacebook} placeholder='facebook url'></TextInputNormal>
                                                            </div>
                                                        </div>
                                                    </Col>
                                                )
                                    ...
                                    <div>
                                                                ...
                                                                <Grid style={{display:'flex'}} container spacing={0}>
                                                                    <Grid style={{ display:'flex', alignItems:'center', justifyContent:'center'}} item xs="auto" >
                                                                        <span style={{marginRight:'5px'}}>
                                                                            <a onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'call'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'call' , channel:`+${e.countryCode}${e.number}`}))}}  href={`tel:${e.countryCode}${e.number}}`}>
                                                                    ...
                                                                    <Grid style={{ display:'flex', alignItems:'center', justifyContent:'center'}} item xs="auto" >
                                                                        {e.whatsApp === true ?
                                                                            <a onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'wa'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'wa' , channel:`+${e.countryCode}${e.number}`}))}} href={`https://api.whatsapp.com/send?phone=${e.number}${e.countryCode}`}>
                                                    ...
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                    <span style={{marginRight:'5px'}}>
                                                                        <a onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'em'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'em' , channel:data.email}))}}  href={`mailto:${data.email}`}>
                                                ...
                                                {instagrams.map((data , i)=>{
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                <span style={{marginRight:'5px'}}>
                                                                    <ExternalLink onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'in'}));dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'in' , channel:data.instagram}))}} href={`https://${data.instagram}`}>
                                                ...
                                                {websites.map((data , i)=>{
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                <span style={{marginRight:'5px'}}>
                                                                    <ExternalLink onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'web'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'web' , channel:data.website}))}}  href={`https://${data.website}`}>
                                                ...
                                                {linkedIns.map((data , i)=>{
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                <span style={{marginRight:'5px'}}>
                                                                    <ExternalLink onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'li'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'li' , channel:data.linkedIn}))}}  href={`https://${data.linkedIn}`}>
                                        ...
                                        {botims !== null?
                                            <div> 
                                                ...
                                                <Row className="g-0" style={{padding:'30px 0px 0px 0px'}}>
                                                    <div className={Style.littleTitleDiv}>
                                                        <LittleOneNormal  text='botims'></LittleOneNormal>
                                                        <hr style={{marginLeft:'8px' , maxWidth:'15px' , padding:"3px , 0px , 3px , 0px"}} className={Style.dashLineVer}></hr>
                                                    </div>
                                                </Row>
                                                {botims.map((data , i)=>{
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                <span style={{marginRight:'5px'}}>
                                                                    <ExternalLink onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'bt'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'bt' , channel:data.botim}))}} href={`https://${data.botim}`}>
                                                                        <IconBotton  backgroundColor='#EFEFEF' text={false} icon={<img style={{width:'25px' , height:'25px'}} src={Botim}></img>}></IconBotton>
                                                                    </ExternalLink>
                                                                </span>
                                                                <span style={{marginLeft:'10px'}}>
                                                                    {data.botim}
                                        ...
                                        {facebooks !== null?
                                            <div> 
                                                ...
                                                <Row className="g-0" style={{padding:'30px 0px 0px 0px'}}>
                                                    <div className={Style.littleTitleDiv}>
                                                        <LittleOneNormal  text='facebooks'></LittleOneNormal>
                                                        <hr style={{marginLeft:'8px' , maxWidth:'15px' , padding:"3px , 0px , 3px , 0px"}} className={Style.dashLineVer}></hr>
                                                    </div>
                                                </Row>
                                                {facebooks.map((data , i)=>{
                                                    return(
                                                        <Col style={{padding:'0px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                                            <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'100px' , alignItems:'center'}}>
                                                                <span style={{marginRight:'5px'}}>
                                                                    <ExternalLink onClick={()=>{dispatch(frequentBtnClickForCrmAxios({authCtx , axiosCtx ,customerId:customerData.customer._id ,  btn:'fa'})); dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:'fa' , channel:data.facebook}))}}  href={`https://${data.facebook}`}>
                                                                        <IconBotton  backgroundColor='blue' text={false} icon={<Facebook sx={{color:'white'}}/>}></IconBotton>
                                                                    </ExternalLink>
                                                                </span>
                                                                <span style={{marginLeft:'10px'}}>
                                                                    {data.facebook}
                            ...
                            <div style={{marginTop:'20px'}}>
                                    {calls.length === 0?
                                        <div style={{display:'flex' , justifyContent:'center' , alignItems:'center', minHeight:'75vh' }}>
                                            <NoData caption="You haven't communicated yet"></NoData>
                                        </div>
                                    :calls.length !== 0?
                                                                    ...
                                                                    <Accordion>
                                                                        ...
                                                                        <Typography sx={{width:'100%'}}>
                                                                            <div className={Style.callDropDownDiv}>
                                                                                <div style={{float:'left' ,display:'flex' , justifyContent:'center' , margin:'0px auto 0px 0px' , textAlign:'right'}}>
                                                                                    <div style={{float:'left', margin:'0px 5px 0px 0px' , textAlign:'left'}}>
                                                                                        {e.platform === 'call'?
                                                                                            <ExternalLink onClick={()=>{dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:e.platform , channel:e.channel}))}} href={`tel:${e.channel}`} style={{marginRight:'4px'}}><IconRender botimSize='24px' size='26px' iconName={e.platform}></IconRender></ExternalLink>
                                                                                        :e.platform === 'wa'?
                                                                                            <ExternalLink onClick={()=>{dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:e.platform , channel:e.channel}))}} href={`https://api.whatsapp.com/send?phone=${e.channel}`} style={{marginRight:'4px'}}><IconRender botimSize='24px' size='26px' iconName={e.platform}></IconRender></ExternalLink>
                                                                                        :
                                                                                            <ExternalLink onClick={()=>{dispatch(actions.setTargetDocForCommunication({docItSelf:customerData.customer , status:true , platform:e.platform , channel:e.channel}))}} href={`https://${e.channel}`} style={{marginRight:'4px'}}><IconRender botimSize='24px'  size='26px' iconName={e.platform}></IconRender></ExternalLink>
                                                                                        }
                                                                                        
                                                                                        {e.communicationStatus === true?
                                                                                            <span className={Style.callStatus}><CallMadeIcon sx={{color:'#009a4a'}}></CallMadeIcon><span className={Style.callStatusText}>{new Date(e.communicationDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' , hour12: false})}</span></span>
                                                                                        :e.communicationStatus === false?   
                                                                                            <span className={Style.callStatus}><CallMissedOutgoingIcon sx={{color:'red'}}></CallMissedOutgoingIcon><span className={Style.callStatusText}>{new Date(e.communicationDate).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' , hour12: false})}</span></span>
                                                                                        :null}
                                                                                    </div>
                                                                                    <div className={Style.callDropDownCallType}  >
                                                                                        {callType.filter(w=>{return w.value === e.communicationReason})[0].pr}
                                                                                        {/* <span>communication reason:</span> */}
                                                                                    </div>
                                                                                </div>
                                                                        ...
                                                                        <Typography>
                                                                            ...
                                                                            <div className={Style.chosenInvoiceDiv}>
                                                                                <span>
                                                                                    <div style={{display:'flex', backgroundColor:'#EFEFEF', marginBottom:'5px',borderRadius:'10px' , alignItems:'center'}}>
                                                                                        <span style={{marginRight:'5px'}}>
                                                                                            {e.platform === 'call'?
                                                                                                <ExternalLink href={`tel:${e.channel}`} style={{marginRight:'4px'}}>
                                                                                                    <IconBotton  backgroundColor='rgb(214, 214, 214)' text={false} icon={<IconRender botimSize='24px' size='26px' iconName={e.platform}></IconRender>}></IconBotton>

                                                                                                </ExternalLink>
                                                                                            :e.platform === 'wa'?
                                                                                                <ExternalLink href={`https://api.whatsapp.com/send?phone=${e.channel}`} style={{marginRight:'4px'}}>
                                                                                                    <IconBotton  backgroundColor='rgb(214, 214, 214)' text={false} icon={<IconRender botimSize='24px' size='26px' iconName={e.platform}></IconRender>}></IconBotton>

                                                                                                </ExternalLink>
                                                                                            :
                                                                                                <ExternalLink href={`https://${e.channel}`} style={{marginRight:'4px'}}>
                                                                                                    <IconBotton  backgroundColor='rgb(214, 214, 214)' text={false} icon={<IconRender botimSize='24px' size='26px' iconName={e.platform}></IconRender>}></IconBotton>
                                                                                                </ExternalLink>
                                                                                            }
                                                                            ...
                                                                            <div className={Style.discriptionDiv}>
                                                                                ...
                                                                                {e.description === ""?
                                                                                    <div className={Style.callDiscription}>
                                                                                       No explanation has been written
                                                                                    </div>                                                                
                                                                                :
                    ...
                    <div style={{margin:'0px auto 0px auto'}} className={Style.showCustomerNav}>
                        <Tabs onChange={handleChange}  value={value} aria-label="icon label tabs example">
                            <Tab  style={{width:'33%'}} icon={<ContactEmergencyIcon />} label="Info" />
                            <Tab  style={{width:'33%'}} icon={<CallIcon />} label="Calls" />
                            <Tab  style={{width:'33%'}} icon={<FeedIcon />} label="Requests" />
                        </Tabs> 
                    </div>  
                </div>
            </div>
