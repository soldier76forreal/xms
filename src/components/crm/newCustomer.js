...
import FindCompanyOrPerson from '../functions/findCompanyOrPerson';
...
const NewCustomerPortal = (props) =>{
    ...
    //facebooks onChange 
    const getFacebook = (e) =>{
        var tempAr = [...facebooks];
        tempAr[e.target.name].facebook = e.target.value;
        setFacebooks([...tempAr]);
    }
    //facebooks onChange 
    const getBotim = (e) =>{
        var tempAr = [...botims];
        tempAr[e.target.name].botim = e.target.value;
        setBotims([...tempAr]);
    }
    //address onChange
    ...
    const saveData = async() =>{
        ...
        if(personOrCompany === null){
            ...
            dataToSend ={
                ...
                botim:botims[0].botim === '' ? null : botims,
                facebook:facebooks[0].facebook === '' ? null : facebooks,
                ...
                if(firstName === '' || lastName === '' || firstName === null || lastName === null || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
                    ...
                    try{
                        ...
                        setTimeout(()=>{
                            ...
                            dispatch(actions.toggleNewCustomer())
                            setLoading(false)
                            
                            const closingSuccessMsgTimeOut = setTimeout(()=>{props.setSuccessToast({status:false , msg:'New customer added'})}, 3000);

                        }, 800)
                        dispatch(actions.setLastAddedCustomer({id:response.data._id,name:`${response.data.personalInformation.firstName} ${response.data.personalInformation.lastName}`}))
                        }catch(err){
                            setLoading(false)
            ...
            if(customerType !== null){
                if(companyName === null || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
                    ...
                    try{
                        ...
                        dispatch(actions.setLastAddedCustomer({id:response.data._id,name:`${response.data.personalInformation.companyName}`}))
                        }catch(err){
                            setLoading(false)
                            dispatch(actions.setShowSnackBar({status:true,msg:err.response.data,type:'error'}))
    ...
    return(
        <Fragment>
            <div style={newCustomer === true?{display:'block'}:{display:'none'}}  className={newCustomer === true? `${Style.newInvoice} ${Style.fadeIn}` : newCustomer === false?`${Style.newInvoice} ${Style.fadeOut}`:null}>
                <div className={Style.topSection}>
                    <div  onClick={()=>{dispatch(actions.toggleNewCustomer());}} className={Style.backBtn}><ArrowBackIos className={Style.arrowIcon} sx={{color:'#000' , fontSize:'30px'}}></ArrowBackIos></div>
                            ...
                            return(
                                <Col style={{padding:'5px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                    <div className={Style.phoneNumberMainDiv}>
                                        ...
                                        <div className={Style.phoneNumberCountry}>
                                            <TextInputNormal name={i} onChange={getFacebook} placeholder='facebook url'></TextInputNormal>
                                        </div>
                                    </div>
                                </Col>
                            )
                            ...
                            return(
                                <Col style={{padding:'5px  0px 5px 0px'}} key={i} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                    <div className={Style.phoneNumberMainDiv}>
                                        ...
                                        <div className={Style.phoneNumberCountry}>
                                            <TextInputNormal name={i} onChange={getBotim} placeholder='botim url'></TextInputNormal>
                                        </div>
                                    </div>
                                </Col>
                            )
