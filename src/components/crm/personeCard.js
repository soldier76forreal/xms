...
import { Business, Call, Person, PlusOne, WhatsApp } from "@mui/icons-material";
...
const platformIcons = (e) =>{
    var platformWithIcon = [
      ...
      return filterTheArryForIcon.text
}
const PersoneCard = (props) =>{
    const MySwal = withReactContent(Swal)
    const authCtx = useContext(AuthContext);
    const axiosGlobal = useContext(AxiosGlobal);
    const [fqBtn , setFqBtn] = useState('')
    var newestItem
    ...
    useEffect(() => {
        const fq = props.data.customer.frequentBtnClick
        if(fq.length === 0)
            setFqBtn('call')
        var modeMap = {};
        var maxEl = fq[0], maxCount = 1;
        ...
        <Fragment>
            <div onClick={(e)=>{props.setShowCustomer({status:true , id:props.data.customer._id});e.stopPropagation()}} className={Style.divCard}>
                    ...
                    <Row>
                        ...
                        <Col style={{marginTop:'3px'}}  xs={12} md={12} lg={12} xl={12} xxl={12}>
                            ...
                            <div className={Style.lastCallSubs}>
                                {props.data.customer.communication.length ===0?
                                    'The communication has not been made'
                                :
                                    `${platformIcons(last_Call.platform)}-${last_Call.channel}-${last_Call.communicationStatus === true ? 'Answered':last_Call.communicationStatus === false ?'Not answered':null}`
                                }
                            </div>     
                        </Col>
                    ...
                    <div className={Style.lastRequestsDiv}>
                        ...
                        <div className={Style.requestItSelf}>
                            {props.data.invoice.length === 0?
                                <span>No request has been issued</span>
                            :
                ...
                <div className={Style.insertDate}>
                    <span style={{marginBottom:'4px' , marginRight:'3px'}}>{props.data.customer.personalInformation.personOrCompany==='com'?<Business sx={{fontSize:'25px'}}></Business>:<Person sx={{fontSize:'25px'}}></Person>}</span>
                    {`submitted by: ${
                    props.data?.generatedBy?.firstName
                        ? `${props.data.generatedBy.firstName} ${props.data.generatedBy.lastName}`
                        : "XMS user"
                    } at ${moment(props.data.customer.insertDate).format("YYYY/MM/DD")}`}               
                </div>
