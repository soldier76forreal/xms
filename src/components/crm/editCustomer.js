...
const EditCustomerPortal = (props) =>{
    ...
    const [personOrCompany , setPersonOrCompany] = useState('pr');
    ...
    useEffect(() => {
        if(props.editCustomer.status === true){
           var result = props.editCustomer.theCustomer;
                ...
                if(result.personalInformation.companyName !==null){
                    setCompanyName(result.personalInformation.companyName)
                }
                if(result.personalInformation.personOrCompany !==null){
                    setPersonOrCompany(result.personalInformation.personOrCompany)
                }
                ...
                if(result.contactInfo.facebooks !==null){
                    setFacebooks([...result.contactInfo.facebooks])
                }else if(result.contactInfo.facebooks ===null){
                    setFacebooks([{facebooks:''}])
                }
                if(result.contactInfo.botims !==null){
                    setBotims([...result.contactInfo.botims])
                }else if(result.contactInfo.botims ===null){
                    setBotims([{botims:''}])
                }
    ...
    const saveData = async() =>{
        ...
        if(personOrCompany === ''){
              ...
              dispatch(actions.setShowSnackBar({status:true,msg:'Select the company type',type:'error'}))
              setLoading(false)
        ...
        if(personOrCompany === 'pr'){
            if(customerType !== null){
                ...
                dispatch(actions.setShowSnackBar({status:true,msg:'Select the customer type',type:'error'}))
            ...
            if(customerType !== null){
                if(companyName === null || phoneNumbers[0].number ==='' || phoneNumbers[0].countryCode ===''){
                      dispatch(actions.setShowSnackBar({status:true,msg:'To add new customer, you have to enter the company name and phone number.',type:'error'}))
                      setLoading(false)
                }else{
            ...
            dispatch(actions.setShowSnackBar({status:true,msg:'Select person or company',type:'error'}))
                    ...
                    <div>
                        <Row className="g-0" style={{padding:'0px'}}>
                            <Col style={{padding:'0px'}} sm={12} md={12} lg={12} xl={12} xxl={12} xs={12}>
                                <div style={{width:'100%'}}> 
                                    <button disabled={loading} style={{fontSize:'15px' , display:'flex' , justifyContent:'center' , alignItems:'center'}} onClick={saveData} className={Style.NormalBtn}>{loading === true ? <CircularProgress size='24px' color='inherit'></CircularProgress>:'Edit'}</button>
                                </div>
                            </Col>
                        </Row>
                    </div>
