...
const ShowTheLinkPortal = (props) =>{
    ...
    const getInvoiceData = async() =>{
        try{
            ...
            console.log(res)
            var temp = []
            const documents = Array.isArray(res?.linkDoc?.document) ? res.linkDoc.document : [];
            const finalArr = Array.isArray(res?.finalArr) ? res.finalArr : [];
            for(var i=0;documents.length > i ; i++){
                if(documents[i].type === 'folder'){
                    const searchFolder = finalArr.find((e) => JSON.stringify(e?.doc?._id) === JSON.stringify(documents[i].id));
                    if(searchFolder !== undefined) temp.push(searchFolder)
                }else if(documents[i].type === 'file'){
                    const searchFile = finalArr.find((e) => JSON.stringify(e?.doc?._id) === JSON.stringify(documents[i].id));
                    if(searchFile !== undefined) temp.push(searchFile)
                }
            }
            setCurrent({name:'XFILE' , id:'root',docs:temp})
            setStack([{name:'XFILE' , id:'root',docs:temp}])
        }catch(err){
            console.log(err);
            setCurrent({name:'XFILE' , id:'root',docs:[]})
        }
    }
    useEffect(() => {
        getInvoiceData()
    ...
    const openFolder = (j) =>{
        const finalArrSource = Array.isArray(filesFolders?.finalArr) ? filesFolders.finalArr : [];
        const find = finalArrSource.find(e=>{return JSON.stringify(e?.doc?._id) ===JSON.stringify(j)})
        if(find === undefined) return;
        const finalFileForms =  []
        const finalFolderForms =  []
        const subFiles = Array.isArray(find.subFiles) ? find.subFiles : [];
        const subFolders = Array.isArray(find.subFolders) ? find.subFolders : [];
        for(var i = 0 ; subFiles.length>i ; i++){
            finalFileForms.push({type:'file',doc:subFiles[i]})
        }
        for(var w = 0 ; subFolders.length>w ; w++){
            finalFolderForms.push({type:'folder',doc:subFolders[w] , subFiles:subFolders[w].subFiles , subFolders:subFolders[w].subFolders})
        }
        const finalArr = finalFolderForms.concat(finalFileForms)
        setCurrent({name:find.doc?.name , id:find.doc?._id ,docs:finalArr})
        setStack(prev => [...prev , {name:find.doc?.name , id:find.doc?._id ,docs:finalArr}])
    ...
    lightbox.init();
    const currentDocs = Array.isArray(current?.docs) ? current.docs : [];
            ...
            :   
                ...
                <div style={{overflowY:'scroll', height:'86vh'}} id="gallery--getting-started" >
                    <div  id="gallery--getting-started">
                        ...
                        {currentDocs.map(e=>{
                            if(e?.doc === undefined) return null;
                            if(e.type === 'folder'){
                                return(
