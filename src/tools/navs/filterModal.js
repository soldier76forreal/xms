...
const  FilterModal = (props) =>{
    ...
    useEffect(() => {
        // filterMemory.mis.filter is Mixed since Phase 6 (Session 42) — fields may be
        // undefined for users without stored MIS filters. Guard with Array.isArray,
        // never spread a possibly-missing value.
        if(filterMis && Object.keys(filterMis).length !== 0 && filterMis.filter){
            ...
            setSentToMis(Array.isArray(filterMis.filter.sentTo)?[...filterMis.filter.sentTo]:[])
            setSentByMis(Array.isArray(filterMis.filter.sentBy)?[...filterMis.filter.sentBy]:[])
        }
    }, [filterMis]);
